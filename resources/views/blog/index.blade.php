            ...
            <table class="w-full border-collapse">
                ...
                <tbody class="divide-y divide-orange-200">
                    @foreach ($blogs as $blog)
                        ...
                        <td class="px-4 py-3 font-semibold text-green-600">{{$blog->title}}</td>
                        <td class="px-4 py-3 text-amber-500">{{Str::limit($blog->description,50)}}</td>
                        <td class="px-4 py-3 text-amber-500">{{$blog->created_at->format("d M, Y")}}</td>
                        <td class="px-4 py-3 flex justify-center space-x-2">
                            <a href="{{route("blog.show", $blog)}}" class="px-3 py-1 bg-blue-600 text-white text-xs font-semibold rounded shadow-md hover:bg-blue-600 transition">
                                👁 View
                            </a>
